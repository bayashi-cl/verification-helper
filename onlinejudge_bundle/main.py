...
def main(args: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument('--release', action='store_true', help='Exclude information for debug in bundled code.')
    parsed = parser.parse_args(args)

    language = onlinejudge_verify.languages.list.get(parsed.path)
    assert language is not None
    sys.stdout.buffer.write(language.bundle(parsed.path, basedir=parsed.iquote[0], options={'include_paths': parsed.iquote, 'release': parsed.release}))
